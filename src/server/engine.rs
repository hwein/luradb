//! Engine-thread bootstrap and shutdown (spec perf/018a A1.1): the storage
//! engines, their background tasks, SHM and the API router all live on the
//! engine thread's tokio-uring runtime.

use crate::{
    api::{self, middleware::ParsedCidr, AppState},
...
/// The booted engine side: the API router the bridge serves, the handles the
/// frontend layers read, and what [`graceful_shutdown`] stops.
pub(super) struct Engine {
    pub(super) router: Router,
    pub(super) auth_cache: Arc<AuthCache>,
    pub(super) trusted_cidrs: Arc<Vec<ParsedCidr>>,
    pub(super) teardown: Teardown,
}

/// Everything [`graceful_shutdown`] stops.
...
// performs their I/O; otherwise the tokio::fs paths in `bootstrap_engine`
// stay in use.
...
async fn start_shm(
    ...
    // `registry` is cloned here, before it moves into AppState in `bootstrap_engine`.
...
/// Boots the engine side up to the API router and SHM (A1.1); the listeners
/// are the frontend's.
pub(super) async fn bootstrap_engine(config: &Arc<LuraConfig>, config_path: &Path) -> anyhow::Result<Engine> {
    ...
    let state = AppState {
        ...
        config_file_loaded: config_path.exists(),
    };
...
/// Stops SHM and the engines, flushes pages, and joins the storage thread.
/// Runs after the listeners and the engine loop are down (A6).
pub(super) async fn graceful_shutdown(teardown: Teardown) {
    let Teardown {
        shm_services,
        shutdown_flag,
        lsm_store,
    ...
    // Registered buffers and files stay alive for the whole run; this drop
    // closes their descriptors (a blocking `close(2)` each).
    drop(io_engine);
